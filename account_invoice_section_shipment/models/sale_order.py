...
class SaleOrder(models.Model):
    ...
    def _create_invoices_shipment(self, grouped=False, final=False, date=None):
        """
    ...
    def _create_invoices(self, grouped=False, final=False):
        ...
        """
        invoice_ids = self._create_invoices_shipment(grouped=grouped, final=final)
        for invoice in invoice_ids:
            if not len(invoice.line_ids.mapped("stock_move_id.picking_id").filtered(lambda p:p.state=='done' and p.picking_type_code=='outgoing')):
                continue
            picking = None
            sequence = 10
            section_lines = []
            ...
            for line in lines:
                if line.stock_move_id and picking != line.stock_move_id.picking_id:
                    ...
                    section_lines.append(
                        (
                            ...
                            0,
                            {
                                "name": picking.get_invoice_section_name(),
    ...
    def _get_ordered_invoice_lines(self, invoice):
        return invoice.line_ids.sorted(key=lambda r: r.stock_move_id.picking_id.id).filtered(lambda r: not r.exclude_from_invoice_tab)
